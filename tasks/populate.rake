namespace :db do
  ...
  task :populate => :environment do
    ...
    (Dir[File.join(Rails.root, 'db', 'populate', RAILS_ENV, '*.rb')] + Dir[File.join(Rails.root, 'db', 'populate', 'shared', '*.rb')]).sort{|x,y| File.basename(x) <=> File.basename(y)}.each do |fixture|
      load fixture 
      puts "Loaded #{fixture}"
    end
